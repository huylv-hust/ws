<form id="login_form" method="post" action="<?= \yii\helpers\BaseUrl::base(true)?>/regist-workslip.html?denpyo_no=<?= (int)$d03DenNo ?>" class="form-horizontal">
<main id="contents">
...
<?= \yii\helpers\Html::dropDownList('M08_NAME_MEI_M08_NAME_SEI', $cus['D01_UKE_JYUG_CD'], $ssUer, array('class' => 'selectForm D01_UKE_JYUG_CD', 'id' => 'D01_UKE_JYUG_CD')) ?>
                    ...
                    <div class="formItem">
                        ...
                        <input type="text" maxlength="6" id="D01_SS_CD" value="<?= $cus['D01_SS_CD'] ?>"
            ...
            <fieldset class="fieldsetRegist">
                ...
                <div class="formGroup">
					...
					foreach ($carOrigin as $key => $carFirst) {
						if (isset($carFirst['D02_DEN_NO']) && $carFirst['D02_DEN_NO'] == 0)
							continue; // check denpyo exit
						?>
						...
						<input type="hidden" name="D02_METER_KM_<?= $k ?>" value="<?= $carFirst['D02_METER_KM'] ?>"/>
                ...
                <div class="formGroup">
                    <div class="formItem">
                        ...
                        <div class="radioGroup">
                            ...
                            <div class="radioItem">
                                <input type="radio" name="D03_KITYOHIN" value="0" id="valuables2" class="radios" <?php if($d03DenNo == 0) echo 'checked="checked"' ;?>>
                                <label
                    ...
                    <div class="formItem">
                        ...
                        <div class="checkGroup">
                            <div class="checkItem" style="position: relative;">
                                <input type="checkbox" name="D03_KAKUNIN" value="1" <?php if ($denpyo['D03_KAKUNIN'] == 1) echo 'checked="checked"' ?> id="agree1" style="display: none">
                                <label class="labelSingleCheck <?php if ($denpyo['D03_KAKUNIN'] == 1) echo 'checked'; ?>" for="agree1">了解済OK</label>
                            </div>
                        </div>
                    </div>
					<!-- BEGIN PAY -->
            ...
            <fieldset class="fieldsetRegist">
                ...
                <div class="formGroup">
                    <div class="formItem">
                        ...
                        <input maxlength="8" type="text" value="<?= $denpyo['D03_SEKOU_YMD'] ?>" name="D03_SEKOU_YMD"
                               class="textForm">
...
<?= \yii\helpers\Html::dropDownList('D03_YOYAKU_SAGYO_NO', $denpyo['D03_YOYAKU_SAGYO_NO'], Yii::$app->params['d03YoyakuSagyoNo'], array('class' => 'selectForm', 'id' => 'D03_YOYAKU_SAGYO_NO')) ?>
                    ...
                    <div class="formItem">
                        <label class="titleLabel">作業者<?php //echo $denpyo['D03_TANTO_MEI'].$denpyo['D03_TANTO_SEI'];?></label>
<?= \yii\helpers\Html::dropDownList('D03_TANTO_MEI_D03_TANTO_SEI', $denpyo['D03_TANTO_MEI'].'[]'.$denpyo['D03_TANTO_SEI'], $ssUerDenpyo, array('class' => 'selectForm D03_TANTO_MEI_D03_TANTO_SEI', 'id' => 'D03_TANTO_MEI_D03_TANTO_SEI')) ?>
                    ...
                    <div class="formItem">
                        <label class="titleLabel">確認者<?php //echo $denpyo['D03_KAKUNIN_MEI'].$denpyo['D03_KAKUNIN_SEI']?></label>
...
foreach ($tm01Sagyo as $work) {
	...
	?>
								<div class="checkItem">
									<input type="checkbox" name="M01_SAGYO_NO[]" id="workDetai<?= $k ?>" value="<?= $work['M01_SAGYO_NO'] ?>" class="checks" <?= $ck ?> >
...
<?php $k = 1; foreach ($listDenpyoCom as $com) { ?>
					<div id="commodity<?= $k ?>"
						 class="commodityBox<?php if ($k == 1 || $k < ($totalDenpyoCom+1)) echo ' on' ?>"><?php if ($k > 1) { ?><a
								class="removeCommodity" href="#">削除</a><?php } ?>
                        ...
                        <div class="formGroup">
							...
							<div class="formItem">
								...
								<p class="txtValue" id="txtValuePrice<?= $k ?>"><?php if(isset($listTm05Edit[$com['D05_COM_CD']])) echo $listTm05Edit[$com['D05_COM_CD']]['M05_LIST_PRICE']?></p>
							</div>
						</div>
						<div class="formGroup">
							<div class="formItem">
								<label class="titleLabel">数量</label>
								<input maxlength="5" type="text" value="<?=$com['D05_SURYO'] ?>" name="D05_SURYO<?= $k ?>" rel="<?= $k ?>" id="no_<?= $k ?>"
									   class="textForm noProduct">
							</div>
							<div class="formItem">
								<label class="titleLabel">単価</label>
								<input maxlength="10" type="text" name="D05_TANKA<?= $k ?>" rel="<?= $k ?>" id="price_<?= $k ?>" value="<?=$com['D05_TANKA'] ?>"
            ...
            <fieldset class="fieldsetRegist">
                ...
                <div class="formGroup">
                    <div class="formItem">
                        ...
                        <div class="checkGroup">
                            <div class="checkItem">
                                <input type="checkbox" name="warranty_check" <?=$ck?> value="1" id="checkWarranty" style="display: none;">
                                <label class="labelSingleCheck <?=$class?>" <?=$style1?> id="checkWarranty" for="checkWarranty">保証書を作成する</label>
                            </div>
                        </div>
                    </div>
                    <div class="formItem">
                        ...
                        <p class="txtValue toggleWarranty" id="warrantyNo" <?=$style?>><?php echo $csv['M09_WARRANTY_NO']?></p>
                    </div>
                    <div class="formItem">
                        <label class="titleLabel">購入日</label>
                        <p class="txtValue toggleWarranty" <?=$style?>><?php if($csv['M09_INP_DATE']) echo $csv['M09_INP_DATE']; else echo  date('Y年m月d日') ?></p>
                    </div>
                    <div class="formItem">
                        <label class="titleLabel">保証期間</label>
                        <p class="txtValue toggleWarranty" id="toggleWarranty" <?=$style?>>
							<?php if($csv['warranty_period'])echo $csv['warranty_period']; else echo date('Y年m月d日',mktime(0, 0, 0, date('m', time()) + 6, date('d', time()), date('Y', time())));?>
							...
							<input type="hidden" name="M09_INP_DATE" id="M09_INP_DATE" value="<?php if($csv['M09_INP_DATE']) echo $csv['M09_INP_DATE']; else echo  date('y-M-d') ?>" />
							<input type="hidden" name="warranty_period" id="warranty_period" value="<?php if($csv['warranty_period'])echo $csv['warranty_period']; else echo date('y-M-d',mktime(0, 0, 0, date('m', time()) + 6, date('d', time()), date('Y', time())));?>" />
						</p>
                    </div>
                </div>
...
<?php for ($i = 1; $i < 5; ++$i) { ?>
					<div class="formGroup lineBottom">
						...
						<div class="formItem">
							<input type="text" value="<?=$csv[$name.'_product']?>" class="textForm" id="<?=$name.'_product'?>" name="<?=$name.'_product'?>">
						</div>
						<div class="formItem">
							<input type="text" value="<?=$csv[$name.'_size']?>" class="textForm" id="<?=$name.'_size'?>" name="<?=$name.'_size'?>">
						</div>
						<div class="formItem">
							<input type="text" value="<?=$csv[$name.'_serial']?>" class="textForm" id="<?=$name.'_serial'?>" name="<?=$name.'_serial'?>">
						</div>
						<div class="formItem">
                    ...
                    <fieldset class="fieldsetRegist">
                        ...
                        <div class="formGroup">
                            ...
                            <div class="formItem">
                                ...
                                <input maxlength="16" type="text" value="<?= trim($cus['D01_KAKE_CARD_NO']) ?>" name="D01_KAKE_CARD_NO"
                        ...
                        <div class="formGroup">
                            <div class="formItem">
                                ...
                                <input maxlength="7" type="text" value="1400002" name="D01_YUBIN_BANGO" class="textForm" id="D01_YUBIN_BANGO">
                        ...
                        <div class="formGroup">
                            <div class="formItem">
                                ...
                                <input type="text" value="<?= $cus['D01_TEL_NO'] ?>" name="D01_TEL_NO" id="D01_TEL_NO"
                                       class="textForm">
                            </div>
                            <div class="formItem">
                        ...
                        <div class="formGroup">
                            <div class="formItem">
                                ...
                                <div class="checkGroup">
                                    <div class="checkItem">
                                        <input type="checkbox" onchange="//agreeForm();" name="agreeCheck" id="agreeCheck"
                                               value="1" class="checks">
                                        <label id="agreeLabel" class="labelSingleCheck" for="agreeCheck">
                                            <a target="_blank"  href="https://verify-ups.com/sss/pdf/PrivacyPolicy.pdf">プライバシーポリシー</a>に同意する
                                        </label>
                                    </div>
                                </div>
                            </div>
...
<?php echo yii\widgets\LinkPager::widget(['pagination' => $pagination]) ?>
            ...
            <div class="modal-footer"><a href="#bgContentProduct" class="btnSubmit" style="cursor: pointer" onclick="closePop()">選択する</a></div>
        </div>
    </div>
</div>
		...
		$.post('<?php yii\helpers\BaseUrl::base(true) ?>registworkslip/default/pdfview',
                {
					...
					'right_front_product':$("#right_front_manu").val(),
					'right_front_manu':$("#right_front_product").val(),
					...
					'M09_WARRANTY_NO':$("#warrantyNo").val(),
            ...
            $.post('<?php yii\helpers\BaseUrl::base(true) ?>registworkslip/default/car',
                    {
                        ...
                        'dataCarApiField': $("#CAR_API_FIELD").val()
                    },
            function (data) {
                if (data.result == '1') {
