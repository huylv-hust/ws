...
//	_selectDay: function(id, month, year, td) {
	...
	_setDateFromField: function(inst, noDefault) {
		...
		if (dates.length > 0) {
			var _y = parseInt(dates.substr(0, 4));
			var _m = parseInt(dates.substr(4, 2));
			if (isNaN(_y) == false && isNaN(_m) == false) {
				_m -= 1;
				inst.drawMonth = inst.selectedMonth = _m;
				inst.drawYear = inst.selectedYear = _y;
			}
		}
...
function isArray(a) {
	return (a && ((false && typeof a == 'object' && a.length) ||
